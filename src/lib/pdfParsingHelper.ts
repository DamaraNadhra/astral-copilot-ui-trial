      ...
      const chunkResults = await Promise.all(
        chunk.map(async (pageText: any, index: number) => {
          const testingResponse = await this.openAISDKHelper.generateObject({
            ...
            2. please take into account the context as well as the layout (if there is) of the page.
            3. when deciding the relevancy please flag out (return a score of 0) any pages that contains unnecessary chapters like introduction, table of contents, etc.`,
